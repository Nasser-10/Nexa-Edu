...
def get_password_hash(password: str) -> str:
    ...
    return "pbkdf2_sha256$310000$" + base64.urlsafe_b64encode(salt).decode() + "$" + base64.urlsafe_b64encode(derived).decode()

def verify_password(plain_password: str, stored: str) -> bool:
    try:
        scheme, rounds, salt_b64, hash_b64 = stored.split("$", 3)
...
def require_role(allowed_roles: list[UserRole]):
    async def role_checker(current_user: User = Depends(get_current_user)) -> User:
        ...
        if current_user.role == UserRole.TEACHER and not current_user.is_teacher_approved:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="حساب المعلم يحتاج إلى اعتماد من الإدارة قبل الوصول إلى أدوات التدريس."
            )
        return current_user
    return role_checker
